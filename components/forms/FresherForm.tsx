...
export default function FresherForm({ department, employmentType, candidateLevel, onBack }: FormProps) {
  const router = useRouter();
  const { config, loading: configLoading } = useTemplateConfig(department, 'fresher');
  ...
  const [probationStipend, setProbationStipend] = useState<number | ''>('');
  ...
  useEffect(() => {
    ...
    if (config.probation_options.length > 0) {
      setProbationPeriod(
        config.probation_options.includes('3 months') ? '3 months' : config.probation_options[0]
      );
    }
  ...
  const validateForm = () => {
    ...
    if (probationPeriod !== 'None' && probationStipend !== '' && Number(probationStipend) < 0) {
      tempErrors.probationStipend = 'Probation stipend cannot be negative';
    }
    ...
    if (!graduationYear || Number(graduationYear) < 1900 || Number(graduationYear) > 2100) {
      tempErrors.graduationYear = 'Enter a valid graduation year';
    }
    ...
    if (!growthPath.trim()) tempErrors.growthPath = 'Please describe the growth path for this fresher role';
    ...
    if (Object.keys(tempErrors).length > 0) {
      ...
      if (tempErrors.ctcMin || tempErrors.ctcMax || tempErrors.probationStipend || tempErrors.bonusDescription) {
        newExpanded.salary = true;
      }
      if (tempErrors.minEducation || tempErrors.preferredBranch || tempErrors.graduationYear || tempErrors.requiredSkills || tempErrors.assignmentDescription) {
        newExpanded.requirements = true;
      }
      if (tempErrors.rolesResponsibilities || tempErrors.whatWeOffer || tempErrors.growthPath || tempErrors.perksBenefits) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        probation_period: probationPeriod === 'None' ? null : probationPeriod,
        probation_stipend: probationPeriod !== 'None' && probationStipend ? Number(probationStipend) : null,
        notice_period: null,
            ...
            {expandedSections.overview && (
              ...
              >
                <div className="p-6 space-y-4">
                  ...
                  {/* Work Mode, Office Location, Shift Timing */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
            ...
            {expandedSections.salary && (
              ...
              >
                <div className="p-6 space-y-4">
                  ...
                  {/* Salary Visibility & Probation Period */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-white/80 mb-2">Probation Period *</label>
                      <select
                        value={probationPeriod}
                        onChange={(e) => setProbationPeriod(e.target.value)}
                        className="w-full bg-[#1A1A1A] border border-white/[0.08] rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C] transition-all"
                      >
                        {(config.probation_options.length > 0
                          ? config.probation_options
                  ...
                  {/* Probation Stipend (Conditional) */}
                  {probationPeriod !== 'None' && (
                    <div id="probationStipend">
                      <label className="block text-sm font-semibold text-white/80 mb-2">Probation Stipend (₹/month) (Optional)</label>
                      <input
                        type="number"
                        min="0"
                        value={probationStipend}
                        onChange={(e) => setProbationStipend(e.target.value ? Number(e.target.value) : '')}
                        placeholder="e.g. 20000 (if different from full scale CTC)"
                        className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                          errors.probationStipend ? 'border-red-500 focus:ring-red-500/30' : 'border-white/[0.08] focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C]'
                        }`}
                      />
                      {errors.probationStipend && <p className="text-xs text-red-500 mt-1.5 flex items-center gap-1"><AlertCircle className="w-3.5 h-3.5" />{errors.probationStipend}</p>}
                    </div>
                  )}
                  ...
                  {performanceBonus && (
                    <div id="bonusDescription">
                      ...
                      <textarea
                        ...
                        placeholder="Provide details about performance payouts, targets, and metrics..."
                        rows={3}
                        className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Stock options, health insurance coverage, component discounts"
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div id="minEducation">
                      ...
                      <input
                        ...
                        placeholder="e.g. B.Tech/B.E in CSE, BCA"
                        className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div id="graduationYear">
                      <label className="block text-sm font-semibold text-white/80 mb-2">Graduation Year *</label>
                      <input
                        type="number"
                        ...
                        className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                          errors.graduationYear ? 'border-red-500 focus:ring-red-500/30' : 'border-white/[0.08] focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C]'
                        }`}
                      />
                      {errors.graduationYear && <p className="text-xs text-red-500 mt-1.5 flex items-center gap-1"><AlertCircle className="w-3.5 h-3.5" />{errors.graduationYear}</p>}
                    </div>
                  </div>
                  ...
                  {assignmentRound && (
                    <div id="assignmentDescription">
                      ...
                      <textarea
                        ...
                        placeholder="Provide details about the test structure, assignment deadlines, or submission method..."
                        rows={3}
                        className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Oracle Java Associate, CCNA, Red Hat Specialist"
                      className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C] transition-all"
                    />
                  </div>
                </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div id="rolesResponsibilities">
                    ...
                    <textarea
                      ...
                      placeholder="Detail the day-to-day work, software/hardware dependencies, and operational contributions..."
                      rows={5}
                      className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                  ...
                  <div id="whatWeOffer">
                    ...
                    <textarea
                      ...
                      placeholder="Mentor systems, state of the art workspace, modern setups, hardware resources, learning budgets..."
                      rows={4}
                      className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                  ...
                  {/* Growth Path */}
                  <div id="growthPath">
                    <label className="block text-sm font-semibold text-white/80 mb-2">Growth Path for Fresher *</label>
                    <textarea
                      value={growthPath}
                      onChange={(e) => setGrowthPath(e.target.value)}
                      placeholder="Career path progression, seniority expectations, promotion patterns in the team..."
                      rows={4}
                      className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                        errors.growthPath ? 'border-red-500 focus:ring-red-500/30' : 'border-white/[0.08] focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C]'
                      }`}
                    />
                    {errors.growthPath && <p className="text-xs text-red-500 mt-1.5 flex items-center gap-1"><AlertCircle className="w-3.5 h-3.5" />{errors.growthPath}</p>}
                  </div>
                  ...
                  <div id="perksBenefits">
                    ...
                    <textarea
                      ...
                      placeholder="Health benefits, fitness access, custom building setup discounts, annual retreats..."
                      rows={3}
                      className={`w-full bg-white/[0.05] border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 transition-all ${
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Any secondary interview details, hiring metrics, contact guidelines..."
                      rows={3}
                      className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C] transition-all"
                    />
                  </div>
                ...
                <div className="p-6 space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Why are you excited to build products in NukePC's domain?"
                      className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C] transition-all"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Describe your experience working in teams or building collaborative tools."
                      className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C] transition-all"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Link to your most complex personal engineering repository."
                      className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-[#FF5A2C]/30 focus:border-[#FF5A2C] transition-all"
                    />
                  </div>
                </div>
